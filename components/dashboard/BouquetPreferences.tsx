...
export default function BouquetPreferences({
    ...
    return (
        <div className="rounded-lg border p-4">
            <h2 className="text-xl font-medium">Предпочтения по букету</h2>
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div>
                    <label className="text-sm text-muted-foreground">Интервал</label>
                    <input
                        className="mt-1 w-full rounded border px-3 py-2"
                        value={interval}
                        onChange={(e) => setInterval(e.target.value)}
                        placeholder="Еженедельно / Раз в две недели"
                    />
                </div>
            </div>
            <div className="mt-4">
